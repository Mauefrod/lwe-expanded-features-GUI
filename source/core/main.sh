...
if [ -f /.flatpak-info ]; then
    ...
    # run_window_manager invokes the configured window manager helper with the provided arguments on the host.
run_window_manager() { flatpak-spawn --host "$WINDOW_MANAGER" "$@"; }
else
    IN_FLATPAK=false
    WINDOW_MANAGER="/usr/local/bin/lwe-window-manager.sh"
    # run_window_manager invokes the configured window manager helper with the provided arguments, routing through the Flatpak host when applicable.
...
# cmd_stop stops all wallpaper engine processes, closes any engine windows, kills the auto-random loop (if running), and removes related state files.
# 
# It first attempts to close detected engine windows gracefully, then forcefully terminates any remaining engine or wrapper processes, stops the background loop using the stored PID (with escalation if needed), and removes saved window and engine state files. Logs actions and tolerates missing/already-terminated processes.
cmd_stop() {

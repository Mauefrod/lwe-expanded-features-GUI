...
if [ -f /.flatpak-info ]; then
    ...
    # xdotool runs the host system's xdotool command via flatpak-spawn --host with the supplied arguments.
...
# get_engine_windows detects engine-related window IDs and prints them, preferring wmctrl output and falling back to PREV_WINDOWS_FILE when wmctrl is unavailable or finds none.
get_engine_windows() {
    local -a windows=()
...
# save_engine_state writes the given engine PID and the provided window ID list to persistent files for later detection.
# The PID is written to "$ENGINE_STATE_FILE.pid" and the window list is written to "$PREV_WINDOWS_FILE".
save_engine_state() {
    local pid="$1"
    local windows="$2"
...
# kill_previous_engine kills running linux-wallpaperengine processes, attempts to close any detected engine windows, and waits briefly to allow termination.
...
# wait_for_window waits for a new engine-related window not present in the provided exclusion list and echoes its window ID; echoes an empty string on timeout.
# It accepts zero or more window IDs to exclude from detection and falls back to a timed assumption when wmctrl is unavailable (Flatpak sandbox).
wait_for_window() {
...
#
# apply_wallpaper launches the wallpaper engine for the specified path, waits for its window, applies window flags, saves engine state, and closes any previous engine windows.
apply_wallpaper() {
    local path="$1"
